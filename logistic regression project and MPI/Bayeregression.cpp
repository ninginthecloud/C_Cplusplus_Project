...
int Length(LPRegression head)
{
    LPRegression current=head;
    int count = 0;
    while(current!=NULL)
    {
     count++;
...
//with predictors A
void AddRegression(int nMaxReg,LPRegression regressions,int lenA,int* A,double logmarglikA)
{
  int i;
  int length;
  ...
  if(length>nMaxReg+1) DeleteLastRegression(regressions);
  printf("inserted [%d]\n",A[0]);
